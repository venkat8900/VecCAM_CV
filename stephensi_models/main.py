...
path = '/home/venkat/Documents/anopheles/'

## Define Path to store the final csv
store_path = '/home/venkat/Documents/results/'

species_all = ["stephensi", "other"]  ## Last class must always be other
...
train_df, test_df = specimen_segregator(path,store_path,"m.jpg",species_all,train_test_split)
...
class MosquitoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = pad(image,(0,0,0))

        label = self.data_df.loc[idx,'label']
        # label = 0 if label=='stephensi' else 1
        self.count+=1
        # print(self.count)
        if self.transform:
...
outputdf.to_csv(output_path+'steph.csv')
